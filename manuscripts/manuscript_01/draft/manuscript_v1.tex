...
\author{Author(s) TBD}
\date{First complete scientific draft -- Manuscript 1.3}
\begin{document}
\maketitle
\begin{abstract}
...
\input{../visuals/tables/M1T01_evidence_planes.tex}
\endgroup
\end{landscape}
\clearpage
...
The F2 analysis treated input admissibility as part of methodological robustness because preprocessing can change whether the AFINO input contract is satisfied. A quality filter can remove a flare peak, a shortened window can leave too few cadences, and filtering can create irregular sampling. Recording these cases explicitly prevents a methodological perturbation from appearing artificially conservative simply because problematic inputs disappear from the analyzed denominator. The same principle later became important at catalogue scale and in the design of the synthetic validation plane.
...
\renewcommand{\_}{\moneoldunderscore\allowbreak}
\renewcommand{\label}[1]{}
\input{../visuals/tables/M1T02_f3a_robustness_counts.tex}
\endgroup
\end{landscape}
\clearpage
...
\renewcommand{\_}{\moneoldunderscore\allowbreak}
\renewcommand{\label}[1]{}
\input{../visuals/tables/M1T03_synthetic_performance.tex}
\endgroup
\end{landscape}
\clearpage
...
The strongest cross-phase robustness result is qualitative continuity rather than a pooled rate. The F2 pilot and F3A catalogue-scale experiment used the same prospectively specified window/profile logic, preserved inadmissibility as an explicit outcome, and both showed that QPP-reference classifications can be retained or lost under methodological perturbation. Scaling therefore did not remove sensitivity to analysis choices. At the same time, F3A exposed a baseline-reproduction limitation that was not present in the small F2 cohort, demonstrating why robustness cannot be assessed responsibly without first checking whether the reference baseline itself reproduces.
...
A useful way to summarize the truth-condition boundary is that observational evidence can test reproducibility without knowing physical truth, whereas synthetic evidence can test classification against known truth without establishing observational realism. Neither plane dominates the other. The observational phases reveal implementation and preprocessing sensitivities that a simplified generator may miss; the synthetic phase supplies confusion-matrix semantics that observational reference labels cannot provide. The manuscript's conclusions rely on their complementarity while refusing to substitute one plane for the missing assumptions of the other.
...
\begin{landscape}
\begingroup
\setlength{\linewidth}{1.16\linewidth}
\setlength{\tabcolsep}{0.8pt}
\renewcommand{\arraystretch}{0.90}
\renewcommand{\small}{\fontsize{6.55}{7.15}\selectfont}
\let\moneoldunderscore\_
\renewcommand{\_}{\moneoldunderscore\allowbreak}
\renewcommand{\label}[1]{}
...
The distinction between characterization and correction is especially important here. A selection surface can be a complete and reproducible summary of a simulation experiment without being a correction function for a survey. Correction requires a target estimand and a defensible mapping from the simulated covariates and latent states to the observational population. The frozen F3B surface was never granted that transport interpretation. Preserving the \texttt{NOT\_ESTABLISHED} correction status therefore keeps the manuscript aligned with what was actually validated under synthetic truth rather than what might be desirable for a later population analysis.
